...
export default class SpaceshipPlayerController implements AI {
    ...
    if (event.type === Homework2Event.PLAYER_DAMAGE) {
      if (event.data.get("shield") === 0) {
        ...
        this.owner.animation.play(
          "shield",
          false,
          Homework2Event.PLAYER_I_FRAMES_END
        );
      }
    }
  }
  ...
  update(deltaT: number): void {
    ...
    let forwardAxis =
      (Input.isPressed("forward") ? 1 : 0) +
      (Input.isPressed("backward") ? -1 : 0);
    let turnDirection =
      (Input.isPressed("turn_ccw") ? -1 : 0) +
      (Input.isPressed("turn_cw") ? 1 : 0);
    ...
    Debug.log(
      "player_pos",
      "Player Position: " + this.owner.position.toString()
    );

    // If the player clicked, we need to spawn in a fleet member
    if (Input.isMouseJustPressed()) {
      this.emitter.fireEvent(Homework2Event.SPAWN_FLEET, {
        position: Input.getGlobalMousePosition(),
      });
    }

    // Animations
    if (
      !this.owner.animation.isPlaying("shield") &&
      !this.owner.animation.isPlaying("explode")
    ) {
      if (this.speed > 0) {
