...
export default class GradientCircleShaderType extends RectShaderType {
  ...
  render(gl: WebGLRenderingContext, options: Record<string, any>): void {
    ...
    let size = new Vec2(maxDimension, maxDimension).scale(
      2 / options.worldSize.x,
      2 / options.worldSize.y
    );

    // Center our translations around (0, 0)
    const translateX =
      (options.position.x - options.origin.x - options.worldSize.x / 2) /
      maxDimension;
    const translateY =
      -(options.position.y - options.origin.y - options.worldSize.y / 2) /
      maxDimension;
    ...
    let transformation = Mat4x4.MULT(
      this.translation,
      this.scale,
      this.rotation
    );
