...
export default class Homework1_Scene extends Scene {
  ...
  private flockControllers: Array<FlockBehavior> = new Array(
    this.MAX_FLEET_SIZE
  );
  ...
  loadScene() {
    ...
    this.load.spritesheet(
      "player",
      "hw2_assets/spritesheets/player_spaceship.json"
    );
    ...
    this.load.spritesheet(
      "fleet",
      "hw2_assets/spritesheets/bunny_spaceship.json"
    );
  }
  ...
  updateScene(deltaT: number) {
    ...
    const paddedViewportSize = this.viewport
      .getHalfSize()
      .scaled(2)
      .add(this.WORLD_PADDING.scaled(2));
  ...
  initializePlayer(): void {
    ...
    this.player.position.set(
      this.viewport.getCenter().x,
      this.viewport.getCenter().y
    );
    ...
    this.player.addAI(SpaceshipPlayerController, {
      owner: this.player,
      spawnFleetEventKey: "spawnFleet",
      initialShield: this.playerShield,
    });
  }
  ...
  initializeUI(): void {
    ...
    // Minerals label
    this.mineralsLabel = <Label>this.add.uiElement(UIElementType.LABEL, "ui", {
      position: new Vec2(125, 50),
      text: `Minerals: ${this.mineralAmount}`,
    });
    ...
    // Shields label
    this.shieldsLabel = <Label>this.add.uiElement(UIElementType.LABEL, "ui", {
      position: new Vec2(375, 50),
      text: `Shield: ${this.playerShield}`,
    });
    ...
    // Fleet label
    this.fleetLabel = <Label>this.add.uiElement(UIElementType.LABEL, "ui", {
      position: new Vec2(625, 50),
      text: "Fleet Size: 0",
    });
    ...
    // Asteroids label
    this.asteroidsLabel = <Label>this.add.uiElement(UIElementType.LABEL, "ui", {
      position: new Vec2(875, 50),
      text: "Asteroids: 0",
    });
  ...
  initializeObjectPools(): void {
    ...
    for (let i = 0; i < this.fleet.length; i++) {
      this.fleet[i] = this.add.animatedSprite(
        Homework2Names.FLEET_SHIP,
        "primary"
      );
      ...
      this.flockControllers[i] = new FlockBehavior(
        this.fleet[i],
        this.fleet,
        150,
        50
      ...
      // Add AI to our ship
      this.fleet[i].addAI(BoidBehavior, {
        fb: this.flockControllers[i],
        separationFactor: 3,
        alignmentFactor: 1,
        cohesionFactor: 3,
      });
    ...
    for (let i = 0; i < this.minerals.length; i++) {
      this.minerals[i] = this.add.graphic(GraphicType.RECT, "primary", {
    ...
    for (let i = 0; i < this.asteroids.length; i++) {
      this.asteroids[i] = this.add.graphic(GraphicType.RECT, "primary", {
        position: new Vec2(0, 0),
  ...
  spawnMineral(): void {
    ...
    if (mineral !== null) {
      ...
      mineral.position = RandUtils.randVec(
        0,
        viewportSize.x,
        0,
        viewportSize.y
      ...
      while (
        mineral.position.distanceTo(this.player.position) <
  ...
  handleTimers(deltaT: number): void {
    ...
    if (this.gameEndTimer > this.GAME_END_MAX_TIME) {
      ...
      this.sceneManager.changeScene(
        GameOver,
        { score: this.numAsteroidsDestroyed },
        {}
      );
    }
  }
  ...
  handleCollisions() {
    ...
    for (let mineral of this.minerals) {
      if (
        mineral.visible &&
        this.player.collisionShape.overlaps(mineral.boundary)
      ) {
      ...
      if (asteroid.visible) {
        for (let ship of this.fleet) {
          ...
          if (
            ...
            Homework1_Scene.checkAABBtoCircleCollision(
              <AABB>ship.collisionShape,
              <Circle>asteroid.collisionShape
            )
          ) {
            // Kill asteroid
    ...
    if (!this.playerinvincible) {
      for (let asteroid of this.asteroids) {
        ...
        if (
          asteroid.visible &&
          Homework1_Scene.checkAABBtoCircleCollision(
            <AABB>this.player.collisionShape,
            <Circle>asteroid.collisionShape
          )
        ) {
          // Put your code here:
  ...
  spawnAsteroid(): void {
    ...
    if (asteroid !== null) {
      ...
      // Loop on position until we're clear of the player
      asteroid.position = RandUtils.randVec(
        0,
        viewportSize.x,
        0,
        viewportSize.y
      ...
      while (
        asteroid.position.distanceTo(this.player.position) <
        this.MIN_SPAWN_DISTANCE
      ) {
        asteroid.position = RandUtils.randVec(
          0,
